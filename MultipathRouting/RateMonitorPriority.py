...
class ProjectController(app_manager.RyuApp):
    ...
    def show_stat(self, type):
        ...
        if(type == 'flow'):
            print('switch         ''          ip-dst      '
                  'out-port packets  bytes    flow-speed(B/s)    flow_speed(Mb/s)')
            print('---------------- ''  ----------------- '
                  '-------- -------- -------- ---------------     -------------')
            for dpid in bodys.keys():
                try:
                    for stat in sorted(
                        [flow for flow in bodys[dpid] if flow.priority == 32768],
                        key=lambda flow: (flow.match.get('ipv4_src'))):
                        print('%016x %17s %8x %8d  %9d   %10.1f             %3.5f' % (
                            dpid,
                            stat.match['ipv4_dst'],
                            ...
                            abs(self.flow_speed[dpid][
                                ...
                                stat.instructions[0].actions[0].port)][-1]), abs(self.flow_speed[dpid][
                                (stat.match.get('ipv4_src'),
                                stat.instructions[0].actions[0].port)][-1]* 8/1000000)))
                        
                        if abs(self.flow_speed[dpid][(stat.match.get('ipv4_src'), stat.instructions[0].actions[0].port)][-1]* 8/1000000) > 8.0:
                            self.overflow.append(dpid)
                except Exception, e:
                    pass
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in sorted([flow for flow in body if flow.priority == 32768], key=lambda flow: (flow.match.get('ipv4_src'))):
            try:
                key = (stat.match.get('ipv4_src'), stat.instructions[0].actions[0].port)
                #print(stat)
    ...
    def get_optimal_paths(self, src, dst, ip_src, ip_dst):
        ...
        if ip_src == self.prior_ip or ip_dst == self.prior_ip:
            ...
            if self.is_prior:
                paths_count = len(paths) if len(paths) < 2 else 2
                if paths_count > 1:
                    return sorted(paths, key=lambda x: self.get_path_cost(x))[1:(paths_count)]
                else:
                    print "No paths available at the moment"
            else:
                paths_count = len(paths) if len(paths) < MAX_PATHS else MAX_PATHS
                return sorted(paths, key=lambda x: self.get_path_cost(x))[0:(paths_count)]
    ...
    def link_delete_handler(self, ev):
        print("link deleted, recomputing paths")
        s1 = ev.link.src
        s2 = ev.link.dst
        try:
    ...
    def del_flow(self, datapath, dst):
        ofproto = datapath.ofproto
        parser = datapath.ofproto_parser
        match = parser.OFPMatch(ipv4_dst=dst)
